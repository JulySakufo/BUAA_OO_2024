...
public class RequestTable {
    ...
    private final ArrayList<NormalResetRequest> normalReset;
    private final ArrayList<DoubleCarResetRequest> doubleCarReset;
    private TransFloor transFloor;
    
    //当前各层等待电梯服务的乘客。<fromFloor,乘客信息>
    public RequestTable() {
        this.requestMap = new HashMap<>();
        this.endFlag = false;
        this.normalReset = new ArrayList<>();
        this.doubleCarReset = new ArrayList<>();
    }
    ...
    public synchronized void addNormalReset(NormalResetRequest normalResetRequest) {
        normalReset.add(normalResetRequest);
        notifyAll();
    }
    
    public synchronized void addDoubleCarReset(DoubleCarResetRequest doubleCarResetRequest) {
        doubleCarReset.add(doubleCarResetRequest);
        notifyAll();
    }
    ...
    public synchronized NormalResetRequest removeNormalResetRequest() {
        notifyAll();
        return normalReset.remove(0);
    }
    
    public synchronized DoubleCarResetRequest removeDCresetRequest() {
        notifyAll();
        return doubleCarReset.remove(0);
    }
    
    public synchronized boolean isNormalResetEmpty() {
        notifyAll();
        return normalReset.isEmpty();
    }
    
    public synchronized boolean isDoubleCarResetEmpty() {
        notifyAll();
        return doubleCarReset.isEmpty();
    }
    ...
    public void printReceiveDC(int id, String type) {
        for (Integer key : requestMap.keySet()) {
            ArrayList<Person> arrayList = requestMap.get(key);
            for (Person person : arrayList) {
                int personId = person.getPersonId();
                TimableOutput.println(String.format("RECEIVE-%d-%d-%s", personId, id, type));
            }
        }
    }
    ...
    public synchronized void setTransFloor(TransFloor transFloor) {
        this.transFloor = transFloor;
    }
    
    public synchronized int getTransFloor() {
        return this.transFloor.getTransFloor();
    }
    
    public synchronized void wakeMeUp() { //唤醒另一个电梯
        notifyAll();
    }
}