...
public class Mono { //最小单元a*x^b*exp(Factor)^c
    ...
    public boolean isFactor() {
        if (exponent.equals(BigInteger.ZERO) && poly.getMonoMap().isEmpty()) { //只有单纯的系数
            ...
            if (coefficient.equals(BigInteger.ONE)) {
                return true;
            } /*else if (coefficient.compareTo(new BigInteger("-1")) == 0) { //-1不行
                return true;
            }*/ else {
                return false;
            }
    ...
    public Poly deriMono() { //a*x^b*exp(factor)求导
        Poly selfPoly = new Poly();
        selfPoly.addMono(this);//将自身转换为poly，用于newPoly2的求导
        Poly resultPoly = new Poly();
        BigInteger newCoe = coefficient.multiply(exponent);
        BigInteger newExp = exponent.add(new BigInteger("-1"));
        Mono newMono1 = new Mono(newCoe, newExp);
        Poly newPoly1 = new Poly(this.poly.getMonoMap());
        newMono1.setPoly(newPoly1);
